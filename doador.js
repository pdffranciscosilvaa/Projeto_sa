document.addEventListener('DOMContentLoaded', function() {
    ...
    listar.addEventListener('click', function() {
        ...
        .then(response => response.json())
        .then(data => {
            let doadoresHTML = '<h2>Lista de Doadores</h2>';
            data.forEach(doador => {
                doadoresHTML += `
                    <div>
                        Nome do Doador: ${doador.nomeDoador}<br>
                        CPF do Doador: ${doador.cpfDoador}<br>
                        CNPJ do Doador: ${doador.cnpjDoador}<br>
                        Endereço do Doador: ${doador.enderecoDoador}<br>
                        Email do Doador: ${doador.emailDoador}<br>
                        Telefone do Doador: ${doador.telefoneDoador}<br>
                        Situação do Doador: ${doador.situacaoDoador}<br>
                        Código do Funcionário: ${doador.codFuncionario}<br>
                        <hr>
                    </div>
                `;
            });
            respostaDiv.innerHTML = doadoresHTML;
        })
        .catch(error => {
